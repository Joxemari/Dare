...
/** Contexto actual del usuario en el check-in (dónde está AHORA). "bed" (en la
 *  cama/en casa a mínima fricción) mapea a la loc de Dare `home`; "gym" a `gym`;
 *  "office" a `home`+`city`. "anywhere" = "Take me somewhere": no está fijo,
 *  acepta que DARE le mande a un sitio (piscina/gym/bosque/…). "travelling" es
 *  un valor heredado (ya no lo ofrece la UI, pero sigue siendo un check-in
 *  guardado válido). */
export type CurrentLoc = "home" | "city" | "park" | "bed" | "office" | "gym" | "travelling" | "anywhere";
